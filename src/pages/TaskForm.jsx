...
export default function TaskForm() {
  ...
  const loadFormData = async () => {
    ...
    if (projectsResult.success) {
      ...
      if (!canCreateTask(user, { id: "any", createdBy: user.id, members: [user.id] })) {
    ...
    if (usersResult.success) {
      setDevelopers(usersResult.data.filter((u) => u.role === "developer" && u.isActive));
    }
    ...
    if (isEdit) {
      ...
      if (taskResult.success) {
        ...
        reset({
          ...
          dueDate: taskResult.data.dueDate.split("T")[0],
        });
      } else {
